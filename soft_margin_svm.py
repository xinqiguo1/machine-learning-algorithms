...
def svm_train_bgd(X: np.ndarray, y: np.ndarray, num_epochs: int=100, C: float=5.0, eta: float=0.001):
    ...
    for j in range(1, num_epochs+1):
        ...
        indicator_w = np.multiply(np.multiply(np.multiply(np.transpose(W @ X.T) + b, y) < 1, y),X)
        indicator_b = np.multiply(np.multiply(np.transpose(W @ X.T) + b, y) < 1, y)
        
        delta_W = W - 1*C*(np.sum(indicator_w, axis = 0))   
