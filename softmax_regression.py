...
def compute_softmax_probs(X: np.ndarray, W: np.ndarray) -> np.ndarray:
    ...
    prob = np.matmul(W, np.transpose(X))
    temp = -np.max(prob, axis = 0)
    numer = np.exp(prob+temp)
    deno = np.sum(numer, axis = 0)
    probs = np.transpose(numer) / deno[:,None]
...
def gradient_ascent_train(X_train: np.ndarray,
    ...
    indicator = class_matrix[int_Y_train - 1]
    ...
    for epoch in range(max_iters):
        ...
        ###################################################################
        #raise NotImplementedError("TODO: Add your implementation here.")
        probs = compute_softmax_probs(X_train, W)
        
        delta_W = np.matmul(np.transpose(indicator-probs),X_train)
#         for i in range(N):
#             indicator = np.zeros((num_class,1), dtype=np.int32)
#             indicator[int(Y_train[i,0])-1] = 1
#             probs = compute_softmax_probs(W, X_train[i, :])
#             #probs = compute_softmax_probs(W, X_train[i, :])
#             #probs = np.reshape(probs, (num_class, 1))


#             delta_W = delta_W + np.multiply(np.tile(X_train[i, :], (num_class, 1)), np.tile((indicator - probs), (1, d)))
...
#         probs = compute_softmax_probs(W, X_test[i, :])
#         idx = np.argmax(probs)
#         if (idx+1 == Y_test[i]):
#             count_correct += 1
